...
class BillingPolicy:
    ...
    def get_quarterly_storage_price(self, storage_record, pi_last_name, start_date):
        ...
        if storage_record.get_storage_start(pi_last_name) > end_date:
            return 0
        storage_amount = storage_record.get_storage_amount(pi_last_name, end_date)
    ...
    def enumerate_quarterly_power_user_prices(
        ...
        for name, user_start, user_end in power_users:
            if (not pd.isna(user_end)) and user_end <= get_end_of_period(
                ...
                price = 0
            elif user_start > get_end_of_period(start_date.year, start_date.month, 1):
                price = 0
    ...
    def generate_quarterly_bill_tex(
        ...
        end_date = get_end_of_period(quarter_start.year, quarter_start.month, 3)
        dates = {
...
class PowerUsersRecord:
    ...
    def describe_user(self, last_name, period_start, period_end):
        ...
        status_updates = relevant_updates.loc[
            relevant_updates["new_power_user"].notna(), ["timestamp", "new_power_user"]
        ]
        ...
        # Status has changed
        end_date = newest_end_date if len(end_date_updates) > 0 else orig_dates[1]
...
def load_user_update_df(user_update_form_path):
    ...
    user_update_df = user_update_df.rename(
        columns={
            ...
            "Last name": "last_name",
            "PI Last Name": "pi_last_name",
            "Request access to additional datashare (specify PI's last name)": "additional_datashare",
            "Update contract end date (account expiration)": "new_end_timestamp",
            "Change account type": "new_power_user",
            "List projects for which you need security access": "new_projects",
            "By clicking yes below, you agree with these general terms.": "agree",
...
def load_pi_df(pi_form_path):
    ...
    pi_df = pi_df.rename(
        columns={
            ...
            + "(There is a fee associated with power user accounts.)": "pi_is_power_user",
...
def load_storage_update_df(storage_update_form_path):
    ...
    storage_update_df = storage_update_df.rename(
        columns={
            ...
            + "If yes, please list the project names.": "access_groups",
            "By clicking yes below, you agree with these general terms.": "agree",
            "Optional: Please feel free to leave any feedback.": "feedback",
        }
    )
...
def add_pis_to_user_df(pi_df, user_df):
    ...
    pi_user_df = pi_df.loc[
        :, ["start_timestamp", "email", "first_name", "last_name", "pi_is_power_user"]
    ]
...
def generate_all_pi_bills(paths, quarter_start_iso, out_dir):
    ...
    for pi_last_name in pi_df.loc[:, "last_name"]:
        out_file = os.path.join(
            out_dir, "pi-{}_quarter-{}_bill.tex".format(pi_last_name, quarter_start_iso)
        )
        generate_pi_bill(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "storage_update_form", type=str, help="path to the storage update form data"
    )
    ...
    generate_all_pi_bills(
        [args.pi_form, args.storage_update_form, args.user_form, args.user_update_form],
        args.quarter_start,
        args.out_dir,
    )
